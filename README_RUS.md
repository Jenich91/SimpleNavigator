...
Реализована библиотеку graph.h_:
* Библиотека разработана на языке С++ стандарта C++17
* Решение оформлено как статическая библиотека (с заголовочным файлом graph.h_)
...
Реализована библиотека graph_algorithms.h_
* Библиотека разработана на языке С++ стандарта C++17
* Библиотека представлена в виде класса `GraphAlgorithms`, который содержат в себе реализацию алгоритмов на графах. При этом сам класс `GraphAlgorithms` не должен ничего знать о внутреннем представлении графа из класса `Graph`. Для взаимодействия с данными графа класс `GraphAlgorithms` может использовать только публичные методы и свойства, предоставляемые классом `Graph`.
...
* Использованы *самописные* вспомогательные классы `Stack` и `Queue` (для этого можно переиспользовать своей решение из проекта *CPP2*). Эти классы содержат в себе следующие методы:
...
* Добавлены в класс `GraphAlgorithms` два новых метода:
...
* Добавлены в класс `GraphAlgorithms` новый метод:
...
* Написана основная программа которая представляет из себя консольное приложение для проверки работоспособности реализованных библиотек sfleta_graph.h и sfleta_graph_algorithms.h
